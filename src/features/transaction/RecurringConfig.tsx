    ...
    <div className={className}>
      <div className="bg-surface-soft border border-hairline p-3 rounded-lg flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between gap-4">
          <Label htmlFor="frequencySelectId" className="text-xs text-ink shrink-0 select-none">重复频率</Label>
          <Select value={value.frequency} onValueChange={handleFrequencyChange}>
            <SelectTrigger id="frequencySelectId" className="w-28 h-8 text-xs bg-canvas border border-hairline">
        ...
        {/* End Date Choice */}
        <div className="flex items-center justify-between gap-4">
          <Label htmlFor="endDateInputId" className="text-xs text-ink shrink-0 select-none">结束日期 (可选)</Label>
          <Input
            id="endDateInputId"
            type="date"
            value={value.endDate || ''}
